...
contract SlashIndicator is ISlashIndicator, System, IParamSubscriber, IApplication {
    ...
    // Persist the count settled at the end of a maintenance session; zero means the session ended in felony.
    function settleMaintenance(address validator, uint256 count) external override onlyValidatorContract onlyInit {
    ...
    function updateParam(string calldata key, bytes calldata value) external override onlyInit onlyGov {
        if (Memory.compareStrings(key, "misdemeanorThreshold")) {
            ...
            uint256 newMaintenanceThreshold = BytesToTypes.bytesToUint256(32, value);
            require(
                newMaintenanceThreshold > 0 && newMaintenanceThreshold < misdemeanorThreshold,
                "the maintenanceThreshold out of range"
            );
            _maintenanceThreshold = newMaintenanceThreshold;
    ...
    /*----------------- query api -----------------*/
    function getSlashIndicator(address validator) external view override returns (uint256, uint256) {
        Indicator memory indicator = indicators[validator];
